...
API_URL=""
for logfile in /tmp/cf_api_tunnel.log /tmp/api_tunnel4.log /tmp/api_tunnel3.log /tmp/api_tunnel2.log /tmp/api_tunnel.log /var/log/cloudflared-bde.log /tmp/tunnel_api.log "/app/data/所有对话/主对话/tunnel_api.log"; do
    [ -f "$logfile" ] || continue
...
MCP_URL=""
for logfile in /var/log/cloudflared-mcp-retry.log /var/log/cloudflared-mcp.log /tmp/cf_mcp_tunnel.log /tmp/mcp_tunnel.log /tmp/tunnel_mcp.log "/app/data/所有对话/主对话/tunnel_mcp.log"; do
    [ -f "$logfile" ] || continue
...
lightbox-essence-complement-learned
burning-until-bath-breeding
retrieve-jobs-congress-made
fantasy-bald-shark-stereo
fantasy-bald-shark-stereo
