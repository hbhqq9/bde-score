...
### v3.0 补充洞察（07-12 18:50）

**Glama.ai 自动收录机制确认：**
- Glama不需要手动提交表单。其收录机制是GitHub爬虫自动发现
- 只需在repo根目录放置`glama.json`（包含`maintainers`字段），Glama约24h内自动收录
- **教训**：v2.0时标记为"需要手动GitHub OAuth提交"是错误判断。正确做法是先调研目标平台的收录机制，而非假设需要手动操作
- **基因强化**：每条新目录提交前，必须先调研其收录方式（API/GitHub爬取/手动表单/CLI），选择最优路径

**Smithery CLI发布路径：**
- Smithery有完整CLI（`smithery mcp publish`），可通过命令行发布
- OAuth认证通过`smithery auth login`获取auth URL，浏览器完成后自动回调
- **v3.0实践**：CLI > 浏览器 > 手动表单（优先使用官方工具链）